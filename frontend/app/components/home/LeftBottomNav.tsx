...
interface LeftBottomNavProps {
  holdCustomer: () => void;
}
const LeftBottomNav: React.FC<LeftBottomNavProps> = ({ holdCustomer }) => {
  const dispatch = useAppDispatch();
  const { tmpCustomerPrescription } = useAppSelector((state) => state.customer);

  type CustomerInfoOnly = Omit<TmpCustomerPrescriptionType, "tmpInvoice">;

  const handlePayment = async () => {
    let obj;
    let { tmpInvoice, ...rest } = tmpCustomerPrescription;
    let invoiceObj: CustomerInfoOnly = rest;

    const customer_prescription: any = [];

    tmpCustomerPrescription.tmpInvoice.forEach((medicine) => {
      // const medicineObj = {
      //   name: medicine.name,
  ...
  return (
    <div className="w-full flex flex-row items-center justify-between gap-x-2">
      ...
      <button className="cursor-not-allowed border border-gray-400 flex flex-row py-1 px-2 w-full justify-center font-semibold text-[#676767] gap-x-1 text-lg rounded">
        <svg
          aria-hidden="true"
          focusable="false"
          className="w-[25px] h-[25px]"
